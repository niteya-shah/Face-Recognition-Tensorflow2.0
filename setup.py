...
## %%
class Config:
    def __init__(self, path_dataset, pre_process=True, as_npy=False,
                 path_preprocessed_dataset=None, img_shape=96, architecture=Architecture.mobile_net):
        path_dataset = Path(path_dataset)
        assert isinstance(img_shape, int)
        self.img_shape = img_shape
        try:
            assert path_dataset.exist()
        ...
        if pre_process:
            ...
            if path_preprocessed_dataset == None:
                ...
                assert path_preprocessed_dataset.exist()

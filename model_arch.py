...
class BaseModel(tf.keras.Model):
    ...
    def __init__(self, values=128):
        ...
        self.conv2d_conv = tf.keras.layers.Conv2D(
            filters, kernel, padding='valid', use_bias=False, strides=(2, 2))
        self.bn_conv = tf.keras.layers.BatchNormalization(axis=channel_axis)
        self.relu_conv = tf.keras.layers.ReLU(6.)

        dpcn2d = tf.keras.layers.DepthwiseConv2D
        ...
        self.den_256 = tf.keras.layers.Dense(256)
        ...
        self.norm_f = tf.keras.layers.Lambda(
            lambda x: tf.math.l2_normalize(tf.cast(x, dtype='float32'),
                                           axis=1))
...
if "__name__" == "__main__":
    ...
    images = tf.keras.layers.Input(
        name="Input", shape=[96, 96, 3], dtype=tf.float32)
    ...
    siamese_model.compile(optimizer=tf.keras.optimizers.Adam(
        0.001), loss=tfa.losses.TripletHardLoss(margin=0.2))
